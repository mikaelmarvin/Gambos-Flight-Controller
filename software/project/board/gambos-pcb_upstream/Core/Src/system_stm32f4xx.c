 ...
 * @brief   CMSIS Cortex-M4 Device Peripheral Access Layer System
 *Source File.
 *
 ...
 *reset and before branch to main program. This call is made inside
...
#define HSE_VALUE                                                    \
    ((uint32_t)25000000) /*!< Default value of the External          \
                            oscillator in Hz */
#endif                   /* HSE_VALUE */
...
/* #define DATA_IN_ExtSRAM */
          ...
          || STM32F469xx || STM32F479xx || STM32F412Zx ||            \
          STM32F412Vx */
...
/* Note: Following vector table addresses must be defined in line with
   linker configuration. */
/*!< Uncomment the following line if you need to relocate the vector
   table anywhere in Flash or Sram, else the vector table is kept at
   the automatic remap of boot address selected */
/* #define USER_VECT_TAB_ADDRESS */

#if defined(USER_VECT_TAB_ADDRESS)
/*!< Uncomment the following line if you need to relocate your vector
   Table in Sram else user remap will be done in Flash. */
...
/* This variable is updated in three ways:
   ...
   system clock frequency Note: If you use this function to configure
   the system clock; then there is no need to call the 2 first
 ...
 * Values. The SystemCoreClock variable contains the core clock
 * (HCLK), it can be used by the user application to setup the SysTick
 * timer or configure other parameters.
 *
 * @note   Each time the core clock (HCLK) changes, this function must
 * be called to update SystemCoreClock variable value. Otherwise, any
 ...
 * the HSE_VALUE(**) or HSI_VALUE(*) multiplied/divided by the PLL
 * factors.
 *
 *         (*) HSI_VALUE is a constant defined in stm32f4xx_hal_conf.h
 * file (default value 16 MHz) but the real value may vary depending
 * on the variations in voltage and temperature.
 *
 *         (**) HSE_VALUE is a constant defined in
 * stm32f4xx_hal_conf.h file (its value depends on the application
 * requirements), user has to ensure that HSE_VALUE is same as the
 * real frequency of the crystal used. Otherwise, this function may
 * have wrong result.
 *
 *         - The result of this function could be not correct when
 * using fractional value for HSE crystal.
 *
 * @param  None
 * @retval None
 */
...
#endif /* STM32F405xx || STM32F415xx || STM32F407xx || STM32F417xx   \
          || STM32F427xx || STM32F437xx || STM32F429xx ||            \
          STM32F439xx || STM32F469xx || STM32F479xx || STM32F412Zx   \
          || STM32F412Vx  */
    (void)(tmp);
}
#endif /* DATA_IN_ExtSRAM && DATA_IN_ExtSDRAM */
