...
HAL_StatusTypeDef HAL_InitTick(uint32_t TickPriority) {
     ...
     * clock. ClockDivision = 0 Counter direction = Up
     */
        ...
        if (status == HAL_OK) {
            ...
            if (TickPriority < (1UL << __NVIC_PRIO_BITS)) {
                ...
                HAL_NVIC_SetPriority(
                    TIM1_UP_TIM10_IRQn, TickPriority, 0U);
 ...
 * @note   Disable the tick increment by disabling TIM1 update
 * interrupt.
 * @param  None
 * @retval None
 */
 ...
 * @note   Enable the tick increment by Enabling TIM1 update
 * interrupt.
 * @param  None
 * @retval None
 */
