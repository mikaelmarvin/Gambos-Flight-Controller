...
/**
 * @brief Picolibc helper function to output a character to a FILE
 * stream. This redirects the output to the low-level __io_putchar
 * function.
...
/**
 * @brief Picolibc helper function to input a character from a FILE
 * stream. This redirects the input from the low-level __io_getchar
 * function.
 * @param file FILE stream pointer (ignored).
...
// FDEV_SETUP_STREAM connects the starm_putc and starm_getc helper
// functions to a FILE structure. _FDEV_SETUP_RW indicates the stream
...
// initialized stream. Picolibc uses these pointers for standard I/O
// operations (printf, scanf, etc.).
FILE *const stdin = &__stdio;
__strong_reference(stdin, stdout);
__strong_reference(stdin, stderr);

// Create strong aliases mapping standard C library function names
// (without underscore) to the implemented system call stubs (with
// underscore). Picolibc uses these standard names internally, so this
// linking is required.
